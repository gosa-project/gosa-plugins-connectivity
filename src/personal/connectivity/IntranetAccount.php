   ...
   */
    function remove_from_parent()
    {
        if ($this->acl_is_removeable() && $this->initially_was_account) {
            ...
            if (!$ldap->success()) {
                msg_dialog::display(__("LDAP error"), msgPool::ldaperror($ldap->get_error(), $this->dn, LDAP_DEL, __CLASS__));
            }
    ...
    function save()
    {
        ...
        if (!$ldap->success()) {
            msg_dialog::display(__("LDAP error"), msgPool::ldaperror($ldap->get_error(), $this->dn, LDAP_MOD, __CLASS__));
        }
    }
    ...
    {
        return (array(
            "plShortName"     => __("Intra-net"),
            "plDepends"       => array("user"),
            "plPriority"      => 25, // Position in tabs
            "plSection"     => array("personal" => __("My account")),
            ...
            "plDescription"       => __("Intra-net account settings") . "&nbsp;(" . __("Connectivity add-on") . ")",
            "plSelfModify"        => TRUE,
    ...
    {
        parent::set_multi_edit_values($values);
